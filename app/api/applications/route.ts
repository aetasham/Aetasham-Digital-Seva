...
export async function POST(request: Request) {
  try {
    ...
    if (!service || !name || !/^\d{10}$/.test(mobile)) {
      return Response.json({ error: 'Please provide a valid service, name and 10-digit mobile number.' }, { status: 400 });
    }
    if (name.length > 80 || details.length > 500) {
    ...
    await put(blobPath(id), JSON.stringify(application), {
      access: 'private',
