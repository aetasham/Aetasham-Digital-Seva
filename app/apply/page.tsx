...
function ApplyForm() {
  ...
  function submit(e: FormEvent) {
    e.preventDefault();
    const id = 'ADS' + Date.now().toString().slice(-8);
    localStorage.setItem('ads_' + id, JSON.stringify({ id, service, name, mobile, details, status: 'Submitted', createdAt: new Date().toISOString() }));
    setResult(id);
    setName('');
    setMobile('');
    setDetails('');
  ...
  return (
    <section className="section">
      <div className="formbox">
        <span className="eyebrow">APPLICATION REQUEST</span>
        <h2>Apply for a Service</h2>
        <p className="muted">This form records a service request for private digital assistance.</p>
